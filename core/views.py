...
class DeliveryRecordViewSet(viewsets.ModelViewSet):
    ...
    def heatmap(self, request):
        ...
        返却: 7x24 行列（weekday=0..6/月..日, hour=0..23）
...
def _hourly_stats_for(dow: int, hour: int, lookback_days: int = 28):
    ...
    ※ 同意ON（UserAiConsent.share_aggregated=True）のユーザーのみ対象
    前提: DeliveryRecord.note の先頭に [AREA:slug] を付与。
    ...
    # 同意ONユーザーのIDセット
    opted_user_ids = set(UserAiConsent.objects.filter(share_aggregated=True).values_list("user_id", flat=True))

    qs = DeliveryRecord.objects.filter(date__gte=since, user_id__in=opted_user_ids)
    ...
    for r in qs.only("date", "earnings", "hours_worked", "start_time", "end_time", "note", "user_id"):
        if r.user_id not in opted_user_ids:
            continue
...
class AreaTodayPlanView(APIView):
    """
    GET /api/areas/today_plan?start=14:00&hours=4&beta=120
    ...
    def get(self, request):
        ...
        if slots:
            ...
            for i, sl in enumerate(slots):
                if sl["slug"] != cur["slug"]:
                    last_end = (datetime.datetime.strptime(slots[i-1]["at"], "%H:%M")
            ...
            # last
            last_end = (datetime.datetime.strptime(slots[-1]["at"], "%H:%M")
                        + datetime.timedelta(hours=1)).strftime("%H:%M")
            cur["end"] = last_end
            blocks.append(cur)
...
from .serializers_consent import AiConsentSerializer  # 別ファイルで用意している前提

class AiConsentView(RetrieveUpdateAPIView):
    """
    GET /api/me/consent/  -> 現状取得
    PUT /api/me/consent/  -> {"share_aggregated": true/false} を更新
    """
    permission_classes = [IsAuthenticated]
    serializer_class = AiConsentSerializer

    def get_object(self):
        obj, _ = UserAiConsent.objects.get_or_create(user=self.request.user)
        return obj
